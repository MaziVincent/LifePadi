...
const AddProductModal = ({
	...
	setOpenAddProductModal: (v: boolean) => void;
}) => {
	const { auth } = useAuth();
	...
	const handleSubmit = async (e: React.FormEvent) => {
		...
		try {
			...
			formData.append("CategoryId", String(Number(category)));
			formData.append("VendorId", String(auth.Id));
	...
	const persistOptions = async (productId: any) => {
		...
		const cleanedE = extras
			.filter((e: any) => e.name?.trim() && e.price !== "")
			.map((e: any) => ({
				ProductId: productId,
				Name: e.name.trim(),
...
const OptionSection = ({
	...
	const addRow = () =>
		setItems((xs: any[]) => [...xs, { name: "", price: "", isDefault: false }]);
	const updateRow = (i: number, patch: any) =>
		setItems((xs: any[]) =>
			xs.map((x, idx) => (idx === i ? { ...x, ...patch } : x)),
		);
	const removeRow = (i: number) =>
		setItems((xs: any[]) => xs.filter((_, idx) => idx !== i));
	const setDefault = (i: number) =>
		setItems((xs: any[]) =>
			xs.map((x, idx) => ({ ...x, isDefault: idx === i })),
		);
