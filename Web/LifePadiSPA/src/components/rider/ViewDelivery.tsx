...
const DeliveredBadge = ({ delivered }: { delivered?: boolean }) =>
	delivered ? (
...
const ViewDelivery = () => {
	...
	const { data, isLoading, isError } = useQuery<any>({
		queryKey: ["delivery", id, auth?.accessToken],
		queryFn: async () => fetch(`${getDeliveryUrl.replace("{id}", id ?? "")}`),
		enabled: !!id,
		retry: 1,
		staleTime: 1000 * 30,
	});
	...
	const logisticQuery = useQuery<any>({
		queryKey: ["logistic", logisticId, auth?.accessToken],
		queryFn: async () => fetch(`/logistics/getByOrder/${logisticId}`),
		enabled: !!logisticId && isLogistic,
		retry: 1,
	});

	const logistic: any = logisticQuery.data;
